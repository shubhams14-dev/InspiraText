import type {
  ...
  class GenerateWithAiButton implements InlineTool {
    ...
    constructor({ api, config }: TemplateInlineToolConstructorOptions) {
      ...
      // Filter undefined and empty objects.
      if (config) {
        this.#config = config;
      }
    }
    ...
    render() {
      ...
      button.type = "button";
      button.innerHTML = this.#config.buttonHTML ?? `Generate with AI`;
      return button;
    }
    ...
    surround() {
      ...
      getAICompletion(selectedText)
        ...
        .catch(() => {
          toast.error("Failed to generate text", { id: "generate-with-ai-editor" });
        })
        .finally(() => {
